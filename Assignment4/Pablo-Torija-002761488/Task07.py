...
# %%
q3 = prepareQuery("""
  SELECT DISTINCT ?personOrAnimal ?properties ?value 
  WHERE { 
    { 
        {
...
q4= prepareQuery("""
  SELECT ?peopleWhoKnowRocky 
  WHERE { 
    {
      ?peopleWhoKnowRocky foaf:knows ?rockyURI.
      ?rockyURI vcard:FN ?rockyFN.
      FILTER(CONTAINS(str(?rockyFN), "Rocky"))
      {
        ?peopleWhoKnowRocky rdf:type ?subClass. 
        ?subClass rdfs:subClassOf ns:Person.
      }
      UNION
      {
        ?peopleWhoKnowRocky rdf:type ns:Person.
      } 
    }
  }
  """,
...
# %%
q5= prepareQuery("""
  SELECT DISTINCT ?entities (COUNT(?otherEntities) as ?numOfEntities) 
  WHERE {
...
# Visualize the results
for r in g.query(q5):
  print(r.entities, r.numOfEntities)